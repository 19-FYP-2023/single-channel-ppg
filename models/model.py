...
class MultiHeadAttention(nn.Module):
    ...
    def positional_encoding_1d(self, d_model, height, width):
        if d_model % 4 != 0:
...
class MultiHeadSelfAttention(MultiHeadAttention):
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        # pe = self.positional_encoding_1d(c, h, w)
        pe = self.pe(x)
        x = x + pe
...
class MultiHeadCrossAttention(MultiHeadAttention):
    def __init__(self, channelY, channelS):
        ...
        self.Sconv = nn.Sequential(
            nn.MaxPool1d(2), nn.Conv1d(channelS, channelS, kernel_size=1),
    ...
    def forward(self, Y, S):
        Sb, Sc, Sh, Sw = S.size()
        Yb, Yc, Yh, Yw = Y.size()
        # Spe = self.positional_encoding_1d(Sc, Sh, Sw)
        Spe = self.Spe(S)
        S = S + Spe
        S1 = self.Sconv(S).reshape(Yb, Sc, Yh * Yw).permute(0, 2, 1)
        V = self.value(S1)
        # Ype = self.positional_encoding_1d(Yc, Yh, Yw)
        Ype = self.Ype(Y)
        Y = Y + Ype
    ...
    def __init__(self, Ychannels, Schannels):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(Ychannels,
                      ...
                      stride=1,
                      padding=1,
                      bias=True), nn.BatchNorm1d(Schannels),
            nn.ReLU(inplace=True),
            nn.Conv1d(Schannels,
                      Schannels,
                      kernel_size=3,
