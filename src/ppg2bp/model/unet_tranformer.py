...
class PositionalEncoding(nn.Module):
    def __init__(self, embedding_size, embedding_length):
        ...
        div_term = 1 / torch.pow(
            10000, torch.arange(0, embedding_length, 2) / embedding_length
        )
...
class AttentionWithPositionalEncoding(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        attention_score = torch.matmul(
            transformed_query, transformed_key.transpose(1, 2)
        )
...
class FeedForwardLayer(nn.Module):
    def __init__(self):
        ...
        self.ffn = nn.Sequential(
            nn.Linear(1024, 1024), nn.GELU(), nn.Linear(1024, 1024)
        )
...
class PPGUnet(nn.Module):
    def __init__(self, in_channels):
        ...
        self.down_conv_layer1 = DoubleConv(
        ...
        self.down_conv_layer2 = DoubleConv(
            in_channels=32,
            out_channels=64,
            first_stride=2,
            second_stride=1,
        ...
        self.down_conv_layer3 = DoubleConv(
            in_channels=64,
            out_channels=128,
            first_stride=2,
            second_stride=1,
        ...
        self.down_conv_layer4 = DoubleConv(
            in_channels=128,
            out_channels=256,
            first_stride=2,
            second_stride=1,
        ...
        self.down_conv_layer5 = DoubleConv(
            in_channels=256,
            out_channels=512,
            first_stride=2,
            second_stride=1,
        ...
        self.posencodingfinal = PositionalEncoding(
            embedding_size=32, embedding_length=1024
        )

        self.attn_layer1 = AttentionWithPositionalEncoding(
            query_dim=1024, key_dim=1024, value_dim=1024, output_dim=1024
        )
        self.attn_layer2 = AttentionWithPositionalEncoding(
            query_dim=512, key_dim=512, value_dim=512, output_dim=512
        )
        self.attn_layer3 = AttentionWithPositionalEncoding(
            query_dim=256, key_dim=256, value_dim=256, output_dim=256
        )
        self.attn_layer4 = AttentionWithPositionalEncoding(
            query_dim=128, key_dim=128, value_dim=128, output_dim=128
        )
        self.attn_layerfinal = AttentionWithPositionalEncoding(
            query_dim=1024, key_dim=1024, value_dim=1024, output_dim=1024
        )

        self.up_conv_layer1 = DoubleConv(
            in_channels=768,
            out_channels=256,
            first_stride=1,
            second_stride=1,
        ...
        self.up_conv_layer2 = DoubleConv(
            in_channels=384,
            out_channels=128,
            first_stride=1,
            second_stride=1,
        ...
        self.up_conv_layer3 = DoubleConv(
            in_channels=192,
            out_channels=64,
            first_stride=1,
            second_stride=1,
        ...
        self.up_conv_layer4 = DoubleConv(
            in_channels=96,
            out_channels=32,
            first_stride=1,
            second_stride=1,
        ...
        self.upsampling_layer1 = UpSample(
            in_channels=512, out_channels=512, scale_factor=2
        )
        self.upsampling_layer2 = UpSample(
            in_channels=256, out_channels=256, scale_factor=2
        )
        self.upsampling_layer3 = UpSample(
            in_channels=128, out_channels=128, scale_factor=2
        )
        self.upsampling_layer4 = UpSample(
            in_channels=64, out_channels=64, scale_factor=2
        )
    ...
    def forward(self, x):
        ...
        attnfinal = self.attn_layerfinal(
            posencoded_final, posencoded_final, posencoded_final
        )
